...
create or replace function public.create_service(
  p_title text,p_date_iso date,p_time text,p_location text default '',p_meeting text default '',p_points integer default 0,
  p_leader_id uuid default null,p_assigned_to uuid default null,p_status public.service_status default 'scheduled')
...
begin
  ...
  values(p_title,p_date_iso,p_time,p_location,p_meeting,p_points,p_leader_id,p_assigned_to,p_status) returning id into v_id;
  return v_id;
end;
$$;
...
create or replace function public.update_service(
  p_service_id uuid,p_title text,p_date_iso date,p_time text,p_location text,p_meeting text,p_points integer,
  p_leader_id uuid,p_assigned_to uuid,p_status public.service_status)
returns boolean language plpgsql security definer set search_path = public
as $$
begin
  if not public.is_admin_or_planner() then raise exception 'Keine Berechtigung'; end if;
  update public.services set title=p_title,date_iso=p_date_iso,time=p_time,location=p_location,meeting=p_meeting,
    points=p_points,leader_id=p_leader_id,assigned_to=p_assigned_to,status=p_status,updated_at=now() where id=p_service_id;
  return found;
end;
$$;
