-- MGB Connect - persistent, database-backed demo mode
-- Demo mode uses isolated demo tables. Real services and announcements are never modified.
...
create policy "authenticated users can read demo services"
...
create policy "planners can manage demo services"
on public.demo_services for all
...
create policy "authenticated users can read demo announcements"
...
create policy "admins can manage demo announcements"
on public.demo_announcements for all
using (public.is_demo_mode() and public.is_admin())
with check (public.is_demo_mode() and public.is_admin());

-- Read-only mode check used by authenticated clients. SECURITY DEFINER avoids
-- exposing all system_settings rows to normal members.
...
-- Recreate the setting row if migration 003 has not inserted it yet.
...
begin
  ...
  if p_enabled = current_enabled then
    return true;
  end if;

  -- Every activation starts from a clean, deterministic dataset. Every
  -- deactivation permanently removes everything that happened in demo mode.
  ...
  if p_enabled then
    ...
    from public.profiles
    where active = true
      and id is distinct from first_user
    order by created_at, id
    limit 1;
    ...
    values
      ('Frühdienst – Demo', current_date + 2, '09:00', 'Demo-Ort', first_user, '08:30 Uhr', 10, 'scheduled', first_user),
    ...
    values
      ('Willkommen im Demo-Modus', 'Alle Dienste und Ankündigungen in diesem Modus sind Testdaten. Änderungen werden gespeichert und beim Ausschalten vollständig zurückgesetzt.', 'general', true, auth.uid()),
      ('Demo-Tauschbörse', 'Dieser Dienst ist absichtlich zur Übernahme freigegeben. Probiere die komplette Tauschbörse mit deinem Account aus.', 'service', true, auth.uid());
  end if;
...
-- Demo service transitions mirror the real service transitions, but can only
-- ever touch the isolated demo_services table.
...
begin
  ...
  set status = 'exchange_requested', taken_by = null, excuse_reason = null, updated_at = now()
...
begin
  if not public.is_demo_mode() then return false; end if;
  update public.demo_services
  set status = 'taken_over', taken_by = auth.uid(), excuse_reason = null, updated_at = now()
...
begin
  ...
  update public.demo_services
  set status = 'exchange_requested', taken_by = null, updated_at = now()
...
begin
  if not public.is_demo_mode() then return false; end if;
  update public.demo_services
  set status = 'excused', excuse_reason = p_reason, taken_by = null, updated_at = now()
  where id = p_service_id and assigned_to = auth.uid() and status = 'scheduled';
  return found;
end;
$$;
...
begin
  if not public.is_demo_mode() or not public.is_admin_or_planner() then return false; end if;
  update public.demo_services
  set status = 'scheduled', excuse_reason = null, taken_by = null, updated_at = now()
