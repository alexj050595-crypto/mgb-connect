...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  const isLeaderAreaActive =
    pathname === "/leader" || pathname.startsWith("/leader/");
      ...
      {open && (
        <>
          ...
          <motion.aside
            ...
            className="fixed left-0 top-0 z-50 flex h-screen max-h-screen w-80 flex-col overflow-hidden border-r border-white/10 bg-white/5 backdrop-blur-lg"
          >
            ...
            <div className="min-h-0 flex-1 overflow-hidden">
              <nav className="h-full overflow-y-auto overflow-x-hidden overscroll-contain px-3 pb-6 pt-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                <div className="flex flex-col gap-1">
                  {navigation.map((item) => {
                    const Icon = item.icon;
                    const active = pathname === item.href;
                    const isLeaderParent =
                      item.href === "/leader" && isLeaderAreaActive;
                    const isAdminParent =
                      item.href === "/admin" && isAdminAreaActive;
                    const showActiveIndicator =
                      active && !isLeaderParent && !isAdminParent;
                    ...
                    return (
                      <Link
                        ...
                        href={item.href}
                        onClick={onClose}
                        className={`group relative flex shrink-0 items-center gap-4 rounded-2xl px-3 py-3 text-left transition-all duration-150 ${
                          showActiveIndicator
                        ...
                        <span
                          className={
                            showActiveIndicator || highlightParent
                              ...
                              : "font-medium text-white/90 transition-colors duration-150"
                          }
                        >
                          {item.title}
                        </span>
