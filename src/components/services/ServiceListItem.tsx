...
export default function ServiceListItem({ id, date, time, title, status }: ServiceListItemProps) {
  return (
    <Link href={`/services/${id}`} className="group block w-full min-w-0 rounded-2xl border border-white/10 bg-white/[0.045] p-4 backdrop-blur-2xl transition hover:border-amber-400/20 hover:bg-white/[0.06] sm:rounded-[28px] sm:p-5">
      <div className="flex flex-col gap-4 sm:gap-5 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex min-w-0 items-center justify-between gap-3 md:justify-end"><StatusBadge status={status} /><ArrowRight size={19} className="shrink-0 text-white/20 transition group-hover:translate-x-0.5 group-hover:text-amber-300" /></div>
      </div>
    </Link>
  );
}
