...
import type { ServiceStatus as ServiceStatusType } from "@/data/services";
...
export default function ServiceCard() {
  ...
  if (!nextService) {
    return (
      <div className="flex h-full min-h-[320px] flex-col rounded-[28px] border border-white/10 bg-white/[0.045] p-6 backdrop-blur-2xl">
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="text-center">
            <CalendarDays size={34} className="mx-auto text-white/30" />
  ...
  return (
    <Link
      ...
      className="group flex h-full min-h-[320px] flex-col rounded-[28px] border border-white/10 bg-white/[0.045] p-6 backdrop-blur-2xl transition-all duration-200 hover:border-amber-400/20 hover:bg-white/[0.065] focus:outline-none focus:ring-2 focus:ring-amber-400/30"
    >
...
function ServiceStatus({ status }: { status: ServiceStatusType }) {
  const config: Record<
    ServiceStatusType,
  ...
  return (
    <div
      className={`inline-flex rounded-xl border px-4 py-2 ${current.wrapper}`}
    >
