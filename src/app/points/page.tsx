...
export default function PointsPage() {
  ...
  return <main className="relative min-h-screen overflow-hidden"><Background /><div className="pointer-events-none fixed inset-x-0 top-0 z-30 h-32 bg-gradient-to-b from-[#050505] via-[#050505]/92 to-transparent" /><Sidebar open={sidebarOpen} onClose={() => setSidebarOpen(false)} /><Topbar sidebarOpen={sidebarOpen} onMenuClick={() => setSidebarOpen(true)} />
    <section className="relative z-10 mx-auto max-w-6xl px-6 pb-16 pt-36">
      ...
      <div className="rounded-[32px] border border-amber-400/20 bg-amber-400/[0.08] p-7 backdrop-blur-2xl"><div className="flex flex-col gap-7 md:flex-row md:items-center md:justify-between"><div><div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-amber-400/20 bg-amber-400/10 text-amber-300"><Trophy size={23} /></div><div><p className="text-sm uppercase tracking-[0.18em] text-amber-300/70">Gesamtpunktestand</p><p className="text-sm text-white/40">Dein aktueller Stand</p></div></div><div className="mt-5 flex items-end gap-3"><span className="text-6xl font-black tracking-tight text-white">{totalPoints}</span><span className="mb-2 text-lg text-white/40">Punkte</span></div></div><div className="grid grid-cols-2 gap-3"><StatBox icon={<CheckCircle2 size={18} />} value={completedServices.length} label="Abgeschlossen" /><StatBox icon={<Star size={18} />} value={averagePoints} label="Ø Punkte" /></div></div></div>
      ...
      <div className="mt-10 rounded-[30px] border border-white/10 bg-white/[0.045] p-6 backdrop-blur-2xl"><div className="flex items-start gap-4"><div className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-white/45"><Trophy size={20} /></div><div className="min-w-0 flex-1"><p className="text-sm uppercase tracking-[0.18em] text-white/35">Rangliste</p><h2 className="mt-1 text-xl font-bold text-white">MGB Rangliste</h2><div className="mt-5 space-y-2">{ranking.map((member, index) => <div key={member.id} className={`flex items-center gap-4 rounded-2xl border p-4 ${member.id === user?.id ? "border-amber-400/20 bg-amber-400/[0.07]" : "border-white/[0.07] bg-black/[0.06]"}`}><span className="w-8 text-center text-sm font-black text-white/35">#{index + 1}</span><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/[0.05] text-white/40"><Trophy size={16} /></div><p className="flex-1 font-semibold text-white">{member.name}{member.id === user?.id ? <span className="ml-2 text-xs font-normal text-amber-300">Du</span> : null}</p><span className="font-bold text-amber-300">{member.points} Pkt.</span></div>)}</div></div></div></div>
